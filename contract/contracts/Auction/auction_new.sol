...
contract ProtonAuction is Ownable {
    ...
    mapping(uint256 => Round) public rounds; // Mapping of rounds
    ...
    constructor(address _protonAddress, address _fundsWalletAdd) Ownable(msg.sender) {
    ...
    function startAuction() external onlyOwner {
        ...
        // Initialize daily auction details for this round
        // for (uint256 i = 1; i <= 7; i++) {
            // round.dailyAuctions[i].tokensDaily = 2_000_000 * 10 ** 18;
            // round.dailyAuctions[i].bonusMultiplier = (i <= 5) ? 3 : (i == 6) ? 2 : 1;
        // }
    }
    ...
    function buyTokens(
        ...
        address userAddress,
        uint256 _amount
     ) external auctionActive {
        uint256 currentDay = getCurrentDay();
        require(currentDay <= 7, "Auction days are complete");

        Round storage round = rounds[currentRound];
        Auction storage auction = round.dailyAuctions[currentDay];

        // require(auction.tokensSold < maxOversubscription, "Not more than limit");
        // require(auction.oversubscribed < maxOversubscriptionlimit, "Not more than limit");

        uint256 usdtAmount = (_amount * tokenPrice) / (10 ** 18);
        require(contractAddress.transferFrom(msg.sender, fundsWallet, usdtAmount), "USDT transfer failed");

        // uint256 amount = _amount;

        // if (auction.tokensSold + _amount > auction.tokensDaily) {
        //     amount = auction.tokensDaily - auction.tokensSold;
        //     auction.oversubscribed += (_amount - amount);
        ...
        uint256 lockTime = block.timestamp + lockingDay;
        PROTON.lock(userAddress, _amount, lockTime);

        require(PROTON.transfer(userAddress, _amount), "Proton token transfer failed");

        emit TokensPurchased(userAddress, usdtAmount, _amount, lockTime);
    }

    function setAuctionStatus(bool status) external onlyOwner {
        isPaused = status;
    }
    ...
    function remainingToken() internal view returns (uint256) {
        return PROTON.balanceOf(address(this));
    }

    // function claimReward(address user, uint256 _usdtAmount) external auctionEnded returns (bool) {
    //     require(userDetail[user].reward > 0 && _usdtAmount <= userDetail[user].reward, "Incorrect value");
    //     userDetail[user].reward -= _usdtAmount;
    //     require(PROTON.transfer(user, _usdtAmount), "Proton token transfer failed");
    //     return true;
    // }
}
