...
const contractAddress = '0xC6BB69e9dc3F85387689d7f0134F7653a665Da32';
const abi = [
...
const ProtonAuction = () => {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                console.log(account)
                const [price, day, [start, end], [tokensDaily, tokensSold, bonusMultiplier], isWhitelister] = await Promise.all([
                    contract.tokenPrice(),
    ...
    const handleApprove = async () => {
        if (usdtContract && metaMaskAddress) {
            try {
                ...
                const allowedAmount = await usdtContract.allowance(metaMaskAddress, contractAddress);
                setAllowance(Number(ethers.formatEther(allowedAmount)));
            } catch (error) {
                setApproving(false);
    ...
    <p>
        <span className="bold-text">Whitelister Status:</span> {isWhitelister ? 'Whitelisted' : 'Not Whitelisted'}
    </p>
            ...
            <div className='buy-token'>
                ...
                <span><p>USDT Balance: {usdtBal}</p></span> 
                ...
                />
                     <span><p>  Allowance: {allowance}</p></span> 
